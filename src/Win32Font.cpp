...
bool CWin32Font::Create(const char *windowsFontName, int tall, int weight, int blur, int scanlines, int flags)
{
	...
	m_iCharset = (flags & vgui::ISurface::FONTFLAG_SYMBOL) ? SYMBOL_CHARSET : DEFAULT_CHARSET;

	if (!stricmp(windowsFontName, "win98japanese"))
	{
		m_iCharset = SHIFTJIS_CHARSET;
		m_szName = "Tahoma";
	}
...
{
	if (!stricmp(windowsFontName, m_szName.String()) && m_iTall == tall && m_iWeight == weight && m_iBlur == blur && m_iFlags == flags)
		return true;
