...
static int pe_write_buffer(struct pe_info *pe, byte *buffer, size_t *len)
{
    int i;
    DWORD file_offset, r;
...
static void pe_build_exports(struct pe_info *pe)
{
    ...
    if (0 == sym_count)
	{
		/* no any export, default export main function (added for nagist) */
        rva_base = pe->thunk->sh_addr - pe->imagebase;
        sym_count = 0, sorted = NULL, op = NULL;
...
ST_FUNC int pe_output_buffer(TCCState * s1, byte **out_buffer, size_t *len)
{
