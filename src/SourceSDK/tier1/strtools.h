...
template <size_t maxLenInChars> void V_wcscpy_safe( wchar_t (&pDest)[maxLenInChars], wchar_t const *pSrc ) 
{ 
...
template <size_t cchDest> char *V_strcat_safe( char (&pDest)[cchDest], const char *pSrc, int nMaxCharsToCopy=COPY_ALL_CHARACTERS )
{ 
	return V_strncat( pDest, pSrc, (int)cchDest, nMaxCharsToCopy ); 
}

wchar_t *V_wcsncat( wchar_t *pDest, const wchar_t *pSrc, size_t cchDest, int nMaxCharsToCopy=COPY_ALL_CHARACTERS );
template <size_t cchDest> wchar_t *V_wcscat_safe( wchar_t (&pDest)[cchDest], const wchar_t *pSrc, int nMaxCharsToCopy=COPY_ALL_CHARACTERS )
{ 
	return V_wcsncat( pDest, pSrc, (int)cchDest, nMaxCharsToCopy ); 
}

char *V_strnlwr( char *pBuf, size_t cchBuf);
template <size_t cchDest> char *V_strlwr_safe( char (&pBuf)[cchDest] )
{ 
...
template <size_t maxLenInCharacters> int V_vsprintfRet_safe( char (&pDest)[maxLenInCharacters], const char *pFormat, va_list params, bool *pbTruncated ) { return V_vsnprintfRet( pDest, maxLenInCharacters, pFormat, params, pbTruncated ); }
...
// Buffer safe wcscat
inline void V_wcscat( wchar_t *dest, const wchar_t *src, int cchDest )
{
...
#define Q_ANSIToUnicode			V_ANSIToUnicode
