...
void *MH_SearchPattern(void *pStartSearch, DWORD dwSearchLen, char *pPattern, DWORD dwPatternLen)
{
	...
	if((DWORD)pStartSearch >= g_dwEngineBase && (DWORD)pStartSearch <= g_dwEngineBase + g_dwEngineSize)
	{
		if(dwEndAddr > g_dwEngineBase + g_dwEngineSize - dwPatternLen)
			dwEndAddr = g_dwEngineBase + g_dwEngineSize - dwPatternLen;
	}
