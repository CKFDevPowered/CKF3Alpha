...
void PM_Jump(void)
{
	...
	if (pmove->flags & FL_LOCK_JUMP)
	{
		pmove->oldbuttons |= IN_JUMP;
		return;
	}

	if((pmove->flags & FL_MULTIJUMP) && !(pmove->oldbuttons & IN_JUMP))
	{
		float flSpeed = 0;
		vec3_t vecVelocity = {0,0,0};
		vec3_t vecHorizon = {0,0,0};
		vec3_t vecForward = {0,0,0};
		vec3_t vecRight = {0,0,0};
		vec3_t vecUp = {0,0,0};
		VectorCopy(pmove->cmd.viewangles, vecHorizon);		
#ifdef CLIENT_DLL
...
#endif
		vecHorizon[0] = 0;
		AngleVectors(vecHorizon, vecForward, vecRight, vecUp);

		if(pmove->cmd.buttons & IN_FORWARD)
		{
			VectorScale(vecForward, flSpeed, vecForward);
			VectorAdd(vecForward, vecVelocity, vecVelocity);
		}
		else if(pmove->cmd.buttons & IN_BACK)
		{
			VectorScale(vecForward, -flSpeed, vecForward);
			VectorAdd(vecForward, vecVelocity, vecVelocity);
		}
		...
		if(pmove->cmd.buttons & IN_MOVELEFT)
		{
			VectorScale(vecRight, -flSpeed, vecRight);
			VectorAdd(vecRight, vecVelocity, vecVelocity);
		}
		else if(pmove->cmd.buttons & IN_MOVERIGHT)
		{
			VectorScale(vecRight, flSpeed, vecRight);
			VectorAdd(vecRight, vecVelocity, vecVelocity);
		}

		if(Length(vecVelocity) > flSpeed)
		{
			VectorNormalize(vecVelocity);
			VectorScale(vecVelocity, flSpeed, vecVelocity);
		}

		vecVelocity[2] = sqrt(2 * 800 * 45.0);

		VectorCopy(vecVelocity, pmove->velocity);
		pmove->fuser2 = 1315.789429;
		PM_FixupGravityVelocity();
		pmove->oldbuttons |= IN_JUMP;
		if(pmove->server)
		{
			pmove->flags &= ~FL_MULTIJUMP;
		}
		return;
	}
...
void PM_RebuildPhysents(struct playermove_s *ppmove)
{
	int i, iPlayerIndex;
	qboolean bShouldCheck;

	iPlayerIndex = ppmove->player_index + 1;
	...
	for(i = 0; i < g_iNumBackupPhysent; i++)
	{
		...
		if(bShouldCheck && PM_NoCollision(iPlayerIndex, g_sBackupPhysent[i].info))
		{
			continue;
		}
...
void PM_ResetPhysents(struct playermove_s *ppmove)
{
