...
CBaseEntity *CBasePlayer::Observer_IsValidTarget(int iTarget, bool bOnlyTeam)
{
	...
	if (!pEnt || pEnt == this || pEnt->m_bDisconnect || pEnt->IsObserver() || (pEnt->pev->effects & EF_NODRAW) || pEnt->m_iTeam == TEAM_UNASSIGNED || (bOnlyTeam && pEnt->m_iTeam != m_iTeam))
		return NULL;
...
void CBasePlayer::Observer_SetMode(int iMode)
{
	...
	if (m_hObserverTarget)
	{
		...
		if (pEnt == this || !pEnt || pEnt->m_bDisconnect || pEnt->IsObserver() || (pEnt->pev->effects & EF_NODRAW) || (forcecamera != FORCECAMERA_SPECTATE_ANYONE && pEnt->m_iTeam != m_iTeam))
			m_hObserverTarget = NULL;
	}
