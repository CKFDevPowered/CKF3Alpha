...
void CSurface::DrawSetTextureFile(int id, const char *filename, int hardwareFilter, bool forceReload)
{
	...
	int bufsize;
	byte *buf = gpRefExports->R_GetTexLoaderBuffer(&bufsize);

	if (gpRefFuncs->LoadTGA(_filename, buf, bufsize, &w, &h))
	{
		DrawSetTextureRGBA(id, buf, w, h, hardwareFilter, forceReload);
	}
}
