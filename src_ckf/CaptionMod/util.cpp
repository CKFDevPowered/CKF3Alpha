...
BOOL UTIL_IsHullDefaultEx(vec3_t vecOrigin, float size, float height)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		/*tr = cl_pmove->PM_PlayerTraceEx(vecOrigin, traceEnds[i], PM_NORMAL, T_BluePrint_IgnoreEnt);
		if (tr.fraction != 1)
			return FALSE;
...
BOOL UTIL_IsHullInZone(physent_t *pe, vec3_t vecMins, vec3_t vecMaxs)
{
	...
	for(int i = 0; i < 8; ++i)
	{
		...
		if(cl_pmove->PM_HullPointContents(hull, hull->firstclipnode, vecStart) == CONTENT_SOLID)
			return TRUE;
	}
	return FALSE;
}
