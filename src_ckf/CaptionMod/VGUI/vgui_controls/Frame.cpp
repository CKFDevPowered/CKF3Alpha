...
void Frame::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, bool hollow /*=false*/ )
{
	...
	if (surface()->IsLockAlpha())
	{
		if (m_bHasFocus)
		{
			int focusAlpha = surface()->GetFocusAlpha();

			if (focusAlpha == 0)
				surface()->SetBlendEnabled(false);
			else if (focusAlpha <= 255)
				surface()->DrawSetAlphaMultiplier(focusAlpha / 255.0);
		}
		else
		{
			int inactiveAlpha = surface()->GetInactiveAlpha();

			if (inactiveAlpha == 0)
				surface()->SetBlendEnabled(false);
			else if (inactiveAlpha <= 255)
				surface()->DrawSetAlphaMultiplier(inactiveAlpha / 255.0);
		}
	}
