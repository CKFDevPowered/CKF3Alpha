...
//-----------------------------------------------------------------------------
bool EditablePanel::LoadControlSettings(const char *resourceName, const char *pathID, KeyValues *pKeyValues)
{
	bool bSuccess = _buildGroup->LoadControlSettings(resourceName, pathID, pKeyValues);
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *EditablePanel::GetFileSchemeFullPath(const char *fileName)
{
	const char *moduleName = GetModuleName();
	const char *schemeName = scheme()->GetSchemeName(moduleName);

	if (schemeName)
	{
		static char fullPath[MAX_PATH];
		sprintf(fullPath, "Resource/Scheme/%s/%s", schemeName, fileName);
		return fullPath;
	}
...
// Purpose: Loads panel settings from a resource file.
//-----------------------------------------------------------------------------
bool EditablePanel::LoadControlSettingsFromScheme(const char *resourceName, const char *pathID, KeyValues *pKeyValues)
{
	bool bSuccess = LoadControlSettings(GetFileSchemeFullPath(resourceName), pathID, pKeyValues);
...
// Purpose: registers a file in the list of control settings, so the vgui dialog can choose between them to edit
//-----------------------------------------------------------------------------
void EditablePanel::RegisterControlSettingsFileEx(const char *resourceName, const char *pathID)
{
	RegisterControlSettingsFile(GetFileSchemeFullPath(resourceName), pathID);
}
...
// Purpose: sets the name of this dialog so it can be saved in the user config area
//-----------------------------------------------------------------------------
void EditablePanel::LoadUserConfigFromScheme(const char *configName, int dialogID)
{
	LoadUserConfig(GetFileSchemeFullPath(configName), dialogID);
}
...
// Purpose: combines both of the above, LoadControlSettings & LoadUserConfig
//-----------------------------------------------------------------------------
void EditablePanel::LoadControlSettingsAndUserConfigFromScheme(const char *dialogResourceName, int dialogID)
{
	LoadControlSettingsAndUserConfig(GetFileSchemeFullPath(dialogResourceName), dialogID);
}
