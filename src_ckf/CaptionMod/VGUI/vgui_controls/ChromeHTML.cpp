...
ChromeHTML::ChromeHTML(Panel *parent, const char *name, bool allowJavaScript, bool bPopupWindow) : Panel(parent, name)
{
	...
	{
		surface()->AccessChromeHTMLController()->CreateBrowser( this, bPopupWindow, "Valve Half-Life" );
	}
	else
	{
...
void ChromeHTML::PerformLayout()
{
	...
	int iSearchTall = 24;
	int iSearchWide = 150;
	const char *resourceString = pClientScheme->GetResourceString( "ChromeHTML.SearchInsetY");
	if ( resourceString )
	{
		iSearchInsetY = atoi(resourceString);
	}
	resourceString = pClientScheme->GetResourceString( "ChromeHTML.SearchInsetX");
	if ( resourceString )
	{
		iSearchInsetX = atoi(resourceString);
	}
	resourceString = pClientScheme->GetResourceString( "ChromeHTML.SearchTall");
	if ( resourceString )
	{
		iSearchTall = atoi(resourceString);
	}
	resourceString = pClientScheme->GetResourceString( "ChromeHTML.SearchWide");
	if ( resourceString )
	{
...
void ChromeHTML::ShowFindDialog()
{
	...
	resourceString = pClientScheme->GetResourceString( "ChromeHTML.SearchAnimationTime");
	if ( resourceString )
	{
...
void ChromeHTML::HideFindDialog()
{
	...
	m_pFindBar->GetBounds( x, y, w, h );
	float flAnimationTime = 0.0f;
	const char *resourceString = pClientScheme->GetResourceString( "ChromeHTML.SearchAnimationTime");
	if ( resourceString )
	{
		flAnimationTime = atof(resourceString);
	}
...
void ChromeHTML::_DeserializeAndDispatch( HTMLCommandBuffer_t *pCmd )
{
	switch ( pCmd->m_eCmd )
	{
	default:
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserReady( const CMsgBrowserReady *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserNeedsPaint( const CMsgNeedsPaint *pCmd )
{
	int tw = 0, tt = 0;
	if ( m_iHTMLTextureID != 0 )
	{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserStartRequest( const CMsgStartRequest *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserURLChanged( const CMsgURLChanged *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserFinishedRequest( const CMsgFinishedRequest *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserShowPopup( const CMsgShowPopup *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserHidePopup( const CMsgHidePopup *pCmd )
{
	HidePopup();
}
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserSizePopup( const CMsgSizePopup *pCmd )
{
	int nAbsX, nAbsY;
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserOpenNewTab( const CMsgOpenNewTab *pCmd )
{
	(pCmd);
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserPopupHTMLWindow( const CMsgPopupHTMLWindow *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserSetHTMLTitle( const CMsgSetHTMLTitle *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserLoadingResource( const CMsgLoadingResource *pCmd )
{
	UpdateCachedHTMLValues();
}
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserStatusText( const CMsgStatusText *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserSetCursor( const CMsgSetCursor *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserFileLoadDialog( const CMsgFileLoadDialog *pCmd )
{
	/*
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserShowToolTip( const CMsgShowToolTip *pCmd )
{
	/*
	BR FIXME
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserUpdateToolTip( const CMsgUpdateToolTip *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserHideToolTip( const CMsgHideToolTip *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserSearchResults( const CMsgSearchResults *pCmd )
{
	if ( pCmd->results() == 0 )
		m_pFindBar->HideCountLabel();
	else
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserClose( const CMsgClose *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserHorizontalScrollBarSizeResponse( const CMsgHorizontalScrollBarSizeResponse *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserVerticalScrollBarSizeResponse( const CMsgVerticalScrollBarSizeResponse *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserGetZoomResponse( const CMsgGetZoomResponse *pCmd )
{
	m_flZoom = pCmd->zoom();
	if ( m_flZoom == 0.0f )
		m_flZoom = 100.0f;
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserLinkAtPositionResponse( const CMsgLinkAtPositionResponse *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserZoomToElementAtPositionResponse( const CMsgZoomToElementAtPositionResponse *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserJSAlert( const CMsgJSAlert *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserJSConfirm( const CMsgJSConfirm *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserCanGoBackandForward( const CMsgCanGoBackAndForward *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::BrowserOpenSteamURL( const CMsgOpenSteamURL *pCmd )
{
...
//-----------------------------------------------------------------------------
void ChromeHTML::SendPendingHTMLMessages()
{
