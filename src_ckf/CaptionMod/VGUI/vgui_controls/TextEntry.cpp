...
void TextEntry::ApplySchemeSettings(IScheme *pScheme)
{
	...
	const char *resourceString = pScheme->GetResourceString("TextEntry.CmpositionBgColor");

	if (resourceString[0])
	{
		_customCmpositionColor = true;
		_cmpositionBgColor = GetSchemeColor("TextEntry.CmpositionBgColor", Color(0, 0, 0, 0), pScheme);
	}
	...
	resourceString = pScheme->GetResourceString("TextEntry.TopLeft");
...
//-----------------------------------------------------------------------------
void TextEntry::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, bool hollow /*=false*/ )
{
...
//-----------------------------------------------------------------------------
int TextEntry::DrawChar(wchar_t ch, HFont font, int index, int x, int y, bool drawSelectBG)
{
	// add to the current position
...
void TextEntry::PaintBackground()
{
		...
		for (i = startIndex; i < endIndex; i++)
		{
			...
			if ( highlight_composition )
			{
				...
				if ( iscompositionchar )
				{
					...
					surface()->DrawSetColor( _customCmpositionColor ? _cmpositionBgColor : col );
	...
	if ( (HasFocus() || (focus && ipanel()->HasParent(focus, GetVParent()))) && m_bAllowNonAsciiCharacters && langlen > 0 )
	{
		wide += m_nLangInset;
		...
		for ( int i = 0; i < langlen; ++i )
		{
			x += DrawChar( shortcode[ i ], _smallfont, i, x, remembery, false );
		}
	}
}
...
void TextEntry::OnInsertText(const wchar_t *wtext)
{
	_cursorIsAtEnd = _putCursorAtEnd;
	_putCursorAtEnd=false;
	...
	// KeyCodes handle all non printable chars
	if (iswcntrl(wtext[0]) || wtext[0] == 9 ) // tab key (code 9) is printable but handled elsewhere
		return;
	
	// do readonly keys
	if (!IsEditable())
	{
		BaseClass::OnKeyTyped(wtext[0]);
		return;
	}
...
void TextEntry::ClickCandidate(int keys)
{
	VPANEL focus = input()->GetFocus();
	bool hasFocus = HasFocus();
	bool childOfFocus = focus && ipanel()->HasParent(focus, GetVPanel());
	...
	if (hasFocus && childOfFocus)
	{
		...
		input[0].ki.wVk = 0x30 + keys;
		SendInput(1, input, sizeof(INPUT));

		input[1].type = INPUT_KEYBOARD;
		input[1].ki.wVk = 0x30 + keys;
...
void TextEntry::ShowIMECandidates()
{
	...
	for ( int i = pageStart; i < pageStart + pageSize; ++i )
	{
		...
		int id = m_pIMECandidates->AddMenuItem( "Candidate", label, new KeyValues("DoClickCandidate", "keys", i - pageStart + startAtOne), this );
		if ( isSelected )
		{
