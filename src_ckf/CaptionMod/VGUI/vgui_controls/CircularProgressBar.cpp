...
void CircularProgressBar::DrawCircleSegment( Color c, float flEndProgress, bool bClockwise )
{
	...
	for ( i=0;i<8;i++ )
	{
		if ( flEndProgressRadians > Segments[i].minProgressRadians )
		{
			...
			// vert 0 is ( 0.5, 0.5 )
			verts[0][0] = flHalfWide;
			verts[0][1] = flHalfTall;
			verts[0][2] = 0.5f;
			verts[0][3] = 0.5f;
			...
			if ( flInternalProgress < SEGMENT_ANGLE )
			{
				...
				verts[2][0] = Segments[i].vert1x * flWide + flDeltaX;
				verts[2][1] = Segments[i].vert1y * flTall + flDeltaY;
				verts[2][2] = Segments[i].vert1x + ( flDeltaX / flHalfWide ) * 0.5;
				verts[2][3] = Segments[i].vert1y + ( flDeltaY / flHalfTall ) * 0.5;
			}
			else
			{
				// full segment, easy calculation
				verts[2][0] = flHalfWide + flWide * ( Segments[i].vert2x - 0.5 );
				verts[2][1] = flHalfTall + flTall * ( Segments[i].vert2y - 0.5 );
				verts[2][2] = Segments[i].vert2x;
				verts[2][3] = Segments[i].vert2y;
			}

			// vert 2 is ( Segments[i].vert1x, Segments[i].vert1y )
			verts[1][0] = flHalfWide + flWide * ( Segments[i].vert1x - 0.5 );
			verts[1][1] = flHalfTall + flTall * ( Segments[i].vert1y - 0.5 );
			verts[1][2] = Segments[i].vert1x;
			verts[1][3] = Segments[i].vert1y;

			vgui::surface()->DrawTexturedPolygon( 3, (float *)verts );
		}
	}
}