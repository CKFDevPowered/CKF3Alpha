...
void main(event_args_t *args)
{
	...
	if (IsLocal(idx))
	{
		...
		CreateSmoke(ent->attachment[0], forward, 3, 0.3, 35, 35, 35, 4, velocity, false, 35);
		CreateSmoke(ent->attachment[0], forward, 35, 0.35, 30, 30, 30, 2, velocity, false, 35);
		CreateSmoke(ent->attachment[0], forward, 70, 0.3, 25, 25, 25, 2, velocity, false, 35);
	}
	...
	EjectBrass(ShellOrigin, ShellVelocity, angles[1], FindModelIndex("models/rshell.mdl"), TE_BOUNCE_SHELL, idx, 17);

	PlaySound(idx, origin, CHAN_WEAPON, "weapons/g3sg1-1.wav", 1.0, 0.6, 0, 94 + RandomLong(0, 15));
