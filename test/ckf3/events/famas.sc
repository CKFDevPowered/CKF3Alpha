...
void main(event_args_t *args)
{
	...
	{
		if (!UseLeftHand())
			GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 17.0, -8.0, -14.0, true);
		else
			GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 17.0, -8.0, 14.0, true);
	...
	EjectBrass(ShellOrigin, ShellVelocity, angles[1], FindModelIndex("models/rshell.mdl"), TE_BOUNCE_SHELL, idx, 8);

	if (RandomLong(0, 1))
		PlaySound(idx, origin, CHAN_WEAPON, "weapons/famas-1.wav", 1.0, 0.48, 0, 94 + RandomLong(0, 15));
	else
		PlaySound(idx, origin, CHAN_WEAPON, "weapons/famas-2.wav", 1.0, 0.48, 0, 94 + RandomLong(0, 15));
