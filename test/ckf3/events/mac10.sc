...
void main(event_args_t *args)
{
	...
	if (IsLocal(idx))
	{
		...
		CreateSmoke(ent->attachment[0], forward, 2, 0.25, 8, 8, 8, 3, velocity, false, 35);
	}

	if (IsLocal(idx))
	{
		if (!UseLeftHand())
			GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 32.0, -9.0, -11.0, false);
		else
			GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 32.0, -9.0, 11.0, false);
	...
	EjectBrass(ShellOrigin, ShellVelocity, angles[1], FindModelIndex("models/rshell.mdl"), TE_BOUNCE_SHELL, idx, 7);

	PlaySound(idx, origin, CHAN_WEAPON, "weapons/mac10-1.wav", 1.0, 0.72, 0, 94 + RandomLong(0, 15));
