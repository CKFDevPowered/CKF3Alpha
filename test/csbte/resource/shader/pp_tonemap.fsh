...
void main() 
{
    vec3 vColor = (1.0f - blurfactor) * texture2D(basetex, gl_TexCoord[0].xy).xyz + blurfactor * texture2D(blurtex, gl_TexCoord[0].xy).xyz;
	...
	gl_FragColor = vec4(pow(vColor.x, contrast), pow(vColor.y, contrast), pow(vColor.z, contrast), 1.0);
}