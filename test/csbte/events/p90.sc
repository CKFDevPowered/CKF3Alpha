...
void main(event_args_t *args)
{
	...
	if (IsLocal(idx))
	{
		...
		CreateSmoke(ent->attachment[0], forward, 3, 0.16, 7, 7, 7, 4, velocity, false, 35);
	}

	if (IsLocal(idx))
	{
		if (!UseLeftHand())
			GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 35.0, -16.0, -22.0, true);
		else
			GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 35.0, -16.0, 22.0, true);
	...
	PlaySound(idx, origin, CHAN_WEAPON, "weapons/p90-1.wav", 1.0, 0.64, 0, 94 + RandomLong(0, 15));
	...
	FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, vSpread, 8192, BULLET_PLAYER_57MM, 2);
}